...
export function buildSummary(rows = []) {
  ...
  const overallProgress = totalCargo > 0 ? (totalDischarge / totalCargo) * 100 : 0
  const averageLoadPerTruck = totalTruck > 0 ? totalDischarge / totalTruck : 0
  const estimatedTruckRequirement =
    totalRemaining > 0 && averageLoadPerTruck > 0 ? Math.ceil(totalRemaining / averageLoadPerTruck) : 0
