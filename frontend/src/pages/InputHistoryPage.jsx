...
function InputHistoryPage({ appState }) {
  ...
  async function loadData() {
    ...
    const assignedResult = await dischargeService.getAssignedVesselsForChecker(checkerId)
  ...
  async function loadEntries() {
    ...
    const entriesResult = await dischargeService.getForChecker(checkerId, {
      page: currentPage,
      pageSize: PAGE_SIZE,
  ...
  return (
    <div className="grid gap-6">
      ...
      {editingEntry && (
        <Modal closeDisabled={isSaving} isOpen={Boolean(editingEntry)} onClose={handleCancelEdit} title="Edit Data Input">
      ...
      <Card
        ...
        subtitle="Gunakan filter untuk menemukan data lama tanpa pindah halaman satu per satu."
      >
        <div className="mb-4 grid gap-4 rounded-lg border border-slate-200 bg-slate-50 p-4">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <Input
              label="Search"
              placeholder="Plat / No Surat Jalan / No SJ"
            ...
            >
              ...
              {getVesselDestinations(selectedVessel).map((destination) => (
                <option key={destination.destinationId} value={destination.destinationId}>
                  {destination.name}
                </option>
              ))}
