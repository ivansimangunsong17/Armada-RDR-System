...
function StowagePlanPage({ appState }) {
  ...
  async function loadData() {
    ...
    const vesselsResult = await vesselService.getAll()

    if (vesselsResult.error) {
      setError('Gagal memuat data kapal dari server.')
      setIsLoading(false)
      return
    }
    ...
    const hatchCargoResult = await vesselService.getHatchCargoByVesselIds(vesselIds)

    if (hatchCargoResult.error) {
      setError('Gagal memuat Final Stowage Plan dari server.')
    } else {
  ...
  async function handleSave() {
    ...
    const saveResult = await vesselService.saveHatchCargo(
      selectedVessel.id,
      hatchRows.map((row) => ({
        hatchNo: row.hatchNo,
    ...
    const deleteResult = await vesselService.deleteExtraHatchCargo(selectedVessel.id, selectedVessel.totalHatch)
    ...
    setMessage('Data Final Stowage Plan berhasil disimpan ke server.')
    setIsSaving(false)
  }
          ...
          >
            ...
            <div className="mt-5 flex flex-col gap-4 rounded-lg bg-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Button type="button" onClick={handleSave} disabled={isSaving}>
                {isSaving ? 'Menyimpan...' : 'Simpan ke Server'}
              </Button>
            </div>
          </Card>
