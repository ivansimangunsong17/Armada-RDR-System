...
function InputMonitoringPage({ appState }) {
  ...
  async function loadVessels() {
    ...
    const result = await reportService.getActiveVessels({
      ...currentUser,
      role: 'admin',
    })
  ...
  async function loadMonitoringData() {
    ...
    const [entriesResult, hatchResult, destinationsResult] = await Promise.all([
      dischargeService.getForVessel(selectedVessel.id, {
        page: currentPage,
        pageSize: PAGE_SIZE,
        ...filters,
      }),
      vesselService.getHatchCargoByVesselIds([selectedVessel.id]),
      vesselService.getVesselDestinations(selectedVessel.id),
    ])
  ...
  async function getAllEntriesForExport() {
    ...
    const result = await dischargeService.getForVessel(selectedVessel.id, filters)
  ...
  async function handleBarcodePhotoChange(event) {
    ...
    setBarcodePhotoPreviewUrl(URL.createObjectURL(compressionResult.data.file))
  }
  ...
  async function handleSubmit(event) {
    ...
    setIsSaving(true)
    setSaveStatus(barcodePhotoFile ? 'uploading' : 'saving')
    setLoadError('')
    setMessage('')
  ...
  return (
    <div className="grid gap-6">
      ...
      {editingEntry && (
        <Modal closeDisabled={isSaving} isOpen={Boolean(editingEntry)} onClose={handleCancelEdit} title="Edit Input Checker">
          <form className="grid gap-4" onSubmit={handleSubmit}>
                ...
                <div className="mt-3">
                  ...
                  {barcodePhotoInfo && (
                    <p className="mt-1 text-sm font-semibold text-slate-700">
                      File baru dipilih: {barcodePhotoInfo.originalName} ({formatFileSize(barcodePhotoInfo.originalSize)}) menjadi {barcodePhotoFile.name} ({formatFileSize(barcodePhotoInfo.compressedSize)})
                    </p>
                  )}
                  {barcodePhotoError && (
            ...
            <div className="flex justify-end gap-3">
              ...
              <Button type="submit" disabled={isSaving}>
                {isSaving ? (saveStatus === 'uploading' ? 'Upload foto...' : 'Menyimpan...') : 'Save Changes'}
              </Button>
            </div>
          </form>
        </Modal>
      ...
      >
        <div className="mb-4 grid gap-4 rounded-lg border border-slate-200 bg-slate-50 p-4">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <Input
              label="Search"
              placeholder="Plate / No Surat Jalan / No SJ"
              value={filters.searchTerm}
              onChange={(event) => updateFilters('searchTerm', event.target.value)}
            />
            <Select
              label="Hatch"
              value={filters.hatchCargoId}
              onChange={(event) => updateFilters('hatchCargoId', event.target.value)}
            >
              <option value="">Semua hatch</option>
              {hatches.map((hatch) => (
                <option key={hatch.id} value={hatch.id}>
                  {hatch.hatchLabel}
                </option>
            ...
            <Select
              ...
              onChange={(event) => updateFilters('destinationId', event.target.value)}
            >
              <option value="">Semua destination</option>
              {vesselDestinations.map((destination) => (
                <option key={destination.destinationId} value={destination.destinationId}>
                  {destination.name}
                </option>
              ))}
            ...
            <Input
              label="Gate Out Date"
              type="date"
              value={filters.gateOutDate}
              onChange={(event) => updateFilters('gateOutDate', event.target.value)}
            />
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <Button type="button" variant="secondary" onClick={goToLatestInput}>
              Go to Latest Input
            </Button>
          </div>
        </div>
