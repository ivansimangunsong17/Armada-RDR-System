...
function PeriodReportPage({ appState }) {
  ...
  async function loadReport() {
    ...
    const result = await reportService.getPeriodTwoHourReport({
    ...
    if (result.error) {
      setError('Gagal memuat report 2 jam dari server.')
    }
    ...
    },
    {
      ...
      render: (row) => (
        <div className="text-right">
          <span className="block">{formatMT(row.remainingCargo)}</span>
          {Number(row.remainingCargo) < 0 && (
            <div className="mt-1">
