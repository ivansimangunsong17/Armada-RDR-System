...
import { buildDestinationSummaryTotal, buildSummary } from '../utils/calculations.js'
...
function formatTruckRequirement(value, remainingCargo) {
  if (Number(remainingCargo) <= 0) return 'Completed'
...
function RunningReportPage({ appState }) {
  ...
  async function loadReport() {
    ...
    const result = await reportService.getDataset(currentUser)

    if (result.error) {
      setError('Gagal memuat running report dari server.')
    }
  ...
  async function loadDestinationSummary(vesselId, isCurrentRequest = () => true) {
    ...
    const result = await reportService.getRunningDestinationSummary(vesselId)
            ...
            <>
              <Card className="print-root p-0">
                ...
                <section className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 sm:p-5 xl:grid-cols-3">
                  {summaryCards.map((item) => (
                    ...
                    >
                      ...
                      <p className="mt-2 wrap-break-word text-lg font-black text-slate-950 sm:text-xl">
                        {item.value}
                      </p>
                    </div>
                  ))}
                      ...
                      >
                        ...
                        <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
                          ...
                          <div>
                            ...
                            <dd className="mt-1 font-black text-slate-950">
                              {formatTruckRequirement(estimatedTruckRequirement, row.remainingOnBoard)}
                            </dd>
                          </div>
                          <div>
                ...
                <div className="mx-4 mb-5 hidden overflow-x-auto rounded-lg border border-slate-200 sm:mx-5 md:block">
                  <table className="w-full min-w-245 table-fixed border-collapse text-sm">
                    <colgroup>
                      <col className="w-[7%]" />
                      <col className="w-[13%]" />
                      <col className="w-[12%]" />
                    ...
                    <tbody>
                      ...
                      <tr className="bg-slate-100 font-extrabold text-slate-950">
                        ...
                        <td className="whitespace-nowrap border border-slate-300 bg-slate-200 px-3 py-3 text-right">
                          {formatTruckRequirement(estimatedTruckRequirementTotal, summaryReport.totalRemaining)}
                        </td>
                        <td className="whitespace-nowrap border border-slate-300 px-3 py-3 text-right">
                          {formatPercentage(summaryReport.overallProgress)}
                        </td>
              ...
              <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
                ...
                <Card className="p-0">
                  ...
                  <div className="hidden overflow-x-auto md:block">
                    <table className="min-w-170 border-collapse text-sm">
