...
import { validateDestinationForm } from '../utils/validators.js'
...
function DestinationPage() {
  ...
  async function loadDestinations() {
    ...
    const result = await vesselService.getDestinations()

    if (result.error) {
      setLoadError('Gagal memuat destination dari server.')
    } else {
  ...
  function validateForm() {
    const nextErrors = validateDestinationForm(form)
  ...
  async function handleSubmit(event) {
    ...
    const result = editingDestination
      ? await vesselService.updateDestination(editingDestination.id, payload)
      : await vesselService.createDestination(payload)
  ...
  async function handleToggleStatus(destination) {
    ...
    const result = await vesselService.changeDestinationStatus(destination.id, !destination.is_active)
  ...
  return (
    <div className="grid gap-6">
      ...
      <Card title="Daftar Destination" subtitle="Data destination berasal dari server.">
        {isLoading ? (
