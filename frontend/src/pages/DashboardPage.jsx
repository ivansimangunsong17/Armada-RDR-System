...
import { buildSummary, buildVesselReports } from '../utils/calculations.js'
import {
  formatDate,
  formatMT,
  formatPercentage,
...
function DashboardPage({ appState, dashboardTitle, dashboardDescription }) {
  ...
  async function loadDashboard() {
    ...
    if (result.error) {
      setError('Gagal memuat dashboard dari server.')
    }
  ...
  return (
    <div className="grid gap-6">
      ...
      {isLoading ? (
        <Card>
          <p className="text-sm text-slate-500">Memuat dashboard dari server...</p>
        </Card>
      ) : (
        <>
          ...
          <Card title="Fleet Overall Progress" subtitle="Progress gabungan semua kapal yang sedang dimonitor.">
            ...
            {Number(fleetSummary.totalRemaining) < 0 && (
              <div className="mt-3">
                <Badge variant="danger">Over Discharge</Badge>
              </div>
            )}
          ...
          <Card title="Progress Semua Kapal" subtitle="Ringkasan progress tiap kapal dari server.">
            ...
            <div className="mb-5 grid gap-4 rounded-lg border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="grid grid-cols-1 gap-3 text-sm text-slate-700 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <div className="rounded-md bg-white p-3 shadow-sm ring-1 ring-slate-200">
                  ...
                  {Number(selectedSummary.totalRemaining) < 0 && (
                    <div className="mt-2">
                      <Badge variant="danger">Over Discharge</Badge>
                    </div>
                  )}
